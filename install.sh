...
install_sprov-ui() {
    ...
    echo -e "检测到sprov-ui最新版本：开始下载核心文件"
    wget -N --no-check-certificate -O /usr/local/sprov-ui/sprov-ui.jar https://github.com/LJea/sprov-ui/releases/download/3.1.0-BAK/sprov-ui.jar
    if [[ $? -ne 0 ]]; then
    ...
    echo -e "wget -O /usr/bin/sprov-ui -N --no-check-certificate https://github.com/LJea/sprov-ui/raw/master/sprov-ui.sh && chmod +x /usr/bin/sprov-ui"
}
