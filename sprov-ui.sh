...
#   Github - sprov-ui: https://raw.githubusercontent.com/LJea/sprov-ui
#======================================================
...
install() {
    install_base
    bash <(curl -L -s https://raw.githubusercontent.com/LJea/sprov-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    bash <(curl -L -s https://raw.githubusercontent.com/LJea/sprov-ui/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/sprov-ui -N --no-check-certificate https://raw.githubusercontent.com/LJea/sprov-ui/raw/master/sprov-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
